...
class DatasetLibri2mix(Dataset):
    def __init__(self,
        tsv_filepath: str = "",
        fs: int = 8000,
    ...
    def __getitem__(self, idx:int):
        ...
        if self.chunk_size <= 0:
            ...
            start = np.random.randint(0, int(mix_info['length'].iloc[0]) - self.chunk_len + 1)
            stop = start + self.chunk_len
            
        # Load wav files
        batch = {}
        batch['mix'] = sf.read(
            mix_info['mixture_path'].iloc[0], start=start, stop=stop, dtype="float32"
        )[0]
        batch['s1'] = sf.read(
            mix_info['source_1_path'].iloc[0], start=start, stop=stop, dtype="float32"
        )[0]
        batch['s2'] = sf.read(
            mix_info['source_2_path'].iloc[0], start=start, stop=stop, dtype="float32"
        )[0]
        if self.use_noise:
            batch['noise'] = sf.read(
                mix_info['noise_path'].iloc[0], start=start, stop=stop, dtype="float32"
            )[0]

        if self.normalize_audio:
            m_std = np.std(batch['mix'], axis=-1, keepdims=True)
            for k in batch.keys():
...
if __name__ == '__main__':
    ...
    cfg.tsv_filepath = '/mnt/xbie/Data/Libri2Mix/wav8k/min/metadata/mnt_mix_dev_mix_clean.csv'
    cfg.fs = 8000
    cfg.chunk_size = 4.0
    cfg.normalize_audio = False

    dataset = DatasetLibri2mix(**cfg)
